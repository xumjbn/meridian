...
- SQLite 落在命名卷 `wjw-data`（`/data/assets.db`），容器重建不丢数据。
...
# 或单独 build：
docker build --build-arg REGISTRY=docker.io -f backend/Dockerfile -t wjw-backend .
```
...
| `WJW_DB` | `/data/assets.db` | SQLite 文件路径；指向挂载卷以持久化（不设则默认当前目录 `assets.db`） |
| `TZ` | `Asia/Shanghai` | 时区（影响调度器与日志时间） |
| `WJW_LOCAL_SHELL` | （未设） | 设 `1` 开启「本机 Shell」本地终端（容器/普通服务端默认关，桌面端默认开） |
...
不想跑容器时，可直接编译出服务端二进制 `wjw-server`，配合任意 nginx 反代。
...
```bash
make server      # 编译服务端二进制 backend/wjw-server（等价 make backend）
make frontend    # 构建前端到 frontend/dist
```

> `make server` 内部用 `CGO_ENABLED=0 GOTOOLCHAIN=local go build -o wjw-server ./cmd/server`。
> 不用 Makefile 也可手动：`cd backend && CGO_ENABLED=0 go build -o wjw-server ./cmd/server`。
...
WJW_DB=/var/lib/wjw/assets.db \
TZ=Asia/Shanghai \
./wjw-server
```

- 不设 `LISTEN_ADDR` 时默认 `127.0.0.1:8080`；不设 `WJW_DB` 时默认在当前工作目录生成 `assets.db`。
- 首启自动建库 + 迁移表结构，并创建默认管理员 **admin / admin**（首次登录强制改密）。
- 需要本机 Shell 本地终端时再加 `WJW_LOCAL_SHELL=1`（普通服务端默认关）。
...
server {
    listen 80;
    server_name _;
    root /var/www/wjw;          # 这里放 frontend/dist 的内容
    index index.html;
...
- **想保留旧的本机裸跑方式** —— 不受影响：不设 `LISTEN_ADDR` / `WJW_DB` 时行为与之前完全一致（监听 `127.0.0.1:8080`，库落当前目录 `assets.db`）。
