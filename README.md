...
| 模块 | 说明 |
|------|------|
| 🛰️ 自动发现 | CIDR / IP 范围并发扫描，端口探测与设备类型指纹识别 |
| 🗂️ CMDB 资产清单 | 资产 CRUD、标签、端口可视化、在线探测、详情抽屉 |
| 🔐 凭据保管箱 | 集中托管 SSH 密码 / 密钥 / Telnet 账号 |
| 💻 WebSSH 终端 | xterm.js + WebSocket 双向交互，自适应缩放、凭据交互、断线提示 |
| 📊 控制台 | 资产态势、存活率、最近活动时间线、实时轮询 |
