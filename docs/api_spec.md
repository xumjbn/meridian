...
- 打开运行后端那台机器的本机 Shell（桌面端/回环自用）。是否可用由 `LocalShellEnabled()` 决定：`WJW_LOCAL_SHELL=1` 显式开启，或后端监听在回环地址时默认开；多用户服务器（`0.0.0.0`）默认关，未启用返回 HTTP 403 文本。前端应先查 `/api/capabilities` 的 `local_shell` 再决定是否展示入口。
