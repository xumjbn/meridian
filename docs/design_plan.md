...
> **文档版本**: v5.1 · 对应应用版本 **v0.33**
> **更新时间**: 2026-06-24
> **状态**: Phase 1–5 已落地；Phase 6（AI Agent 自动化 + 终端体验增强）已落地
...
| v0.32 | 文件管理 UX（去大拖拽框）+ 提示/占位文字对比度 |
| **v0.33** | **悬浮 AI 面板 + 历史对话 + 会话持久化**；终端 **Ctrl+F 搜索 / 配色主题 / 重连历史**；**凭据自动绑定**；手动资产端口；分组折叠修复 |

### 命名与品牌

**Lynx（猞猁线）** 是航海与测绘中的基准经线——呼应「为网络资产测绘、定位、导航」的核心价值，
...
| WebSSH / Telnet 终端 | xterm.js + WebSocket + SSH/Telnet 代理 + 凭据交互 + 重连、应用内多标签、全屏、滚动回看、**多屏分屏（独立关闭 + 自由拖拽缩放）**、**命令同步广播**、**命令自动补全**、**屏幕搜索(Ctrl+F)**、**配色主题(7 套)**、**重连历史回放**、**凭据自动绑定** | 会话录制 |
| AI 命令助手 / Agent | **生成单条命令人工确认**；**Agent 自动执行**（独立 SSH 通道、读输出推进、高危拦截）、**多轮上下文**、**悬浮可展开面板 + 历史对话切换**、**会话持久化** | 逐步流式(SSE) |
...
| AgentSession | agent_sessions | id, requester_id, asset_id, asset_name, title, work_dir, messages(JSON), steps(JSON), status, summary | ✅ |

> `owner_id` 驱动多租户隔离；`ssh_port` 支持非标端口；`arch`/`virtualization` 由认证采集写入；`agent_sessions` 写穿持久化（AI Agent 重启不丢 + 历史对话）。完整接口详见 [api_spec.md](api_spec.md)。
...
| POST | `/api/ai/agent/{start,continue,message}` | AI Agent 自动执行 / 确认 / 多轮 |
| GET | `/api/ai/agent/sessions` · `/:id` | Agent 历史会话列表 / 读取 |
| GET/POST | `/api/assets/:id/sftp/{list,download,upload,mkdir,remove,rename}` | SFTP 文件管理 |
| GET | `/api/activity/recent` | 最近操作活动 |
| WS | `/api/ws/terminal/:id` | WebSSH / Telnet 终端（`autotry` 自动绑定凭据） |
...
### Phase 6：AI Agent 自动化 · 终端体验增强 ✅（v0.31–v0.33）
- [x] **AI Agent**：一句话自动完成任务，后端独立 SSH 通道逐条执行（pwd 跨命令保留、每步超时）、读输出推进、**高危拦截**、**多轮上下文**
- [x] **AI 悬浮面板**：右下角悬浮可展开、可拖拽调宽、目标资产选择、**历史对话切换**
- [x] **会话持久化**：`AgentSession` 写穿落库（重启不丢）+ `GET /ai/agent/sessions` 历史
- [x] **终端体验**：Ctrl+F **屏幕搜索**、7 套**配色主题**热切换、**重连历史回放**
- [x] **凭据自动绑定**：连无凭据资产时逐个试已存 SSH 凭据，成功自动绑定 + 审计
- [x] **修补**：手动录入资产开放端口、分组折叠状态跨维度记忆、文件管理 UX、提示文字对比度
...
- [ ] **登录**会话外置存储（auth.go 令牌仍存进程内存，重启需重新登录；注：AI Agent 会话已落库持久化）
...
> ✅ 已落地（曾在本路线图）：服务端真实鉴权 + 会话校验、多用户与 RBAC、**AI 助手多轮对话与上下文记忆**、**AI Agent 会话持久化**。
...
│       │   ├── uptime.go                  # 在线率历史
│       │   ├── ai.go                      # AI 命令助手（单条生成）
│       │   ├── ai_agent.go                # AI Agent（自动执行 + 会话持久化 + 历史）
│       │   └── notify.go                  # 告警测试
