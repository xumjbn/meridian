...
> 本目录是 wjw 仓库的子模块，独立的整体说明请见仓库根 [`../README.md`](../README.md)。
...
└── src-tauri/                  # Tauri v2 桌面端工程（Rust）
    ├── tauri.conf.json         # productName=wjw，identifier=cn.wjw.desktop
    ├── Cargo.toml / build.rs / src/
    ├── binaries/               # Go 后端 sidecar（wjw-backend）
    ├── capabilities/ / icons/
```
...
同一套前端可打包为桌面客户端：Tauri v2（Rust）将 Go 后端编译为 sidecar（`wjw-backend`）
...
注意底层技术标识（`cn.wjw.desktop`、`wjw-backend` 等）沿用历史命名，不属于品牌名，请勿改动。
