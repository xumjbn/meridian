...
export const TerminalPage: React.FC<TerminalPageProps> = ({ assetId, sessionId, embedded = false, onClose, onOpenSettings }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('wjw-term-help', onHelp);
    return () => window.removeEventListener('wjw-term-help', onHelp);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('wjw-open-in-split', handler);
    return () => window.removeEventListener('wjw-open-in-split', handler);
...
const TerminalItem: React.FC<TerminalItemProps> = ({ paneId, assetId, fontSize, fontFamily, termTheme, termEncoding, assets, completionEnabled, canClose, onClose, onAssetChange, onZoomFont, onResetFont, onActivity, isMaximized, onToggleMaximize, onSplit, onCloseTab, hoistBar = false }) => {
  ...
  useEffect(() => {
    ...
    term.attachCustomKeyEventHandler((e) => {
      ...
      if (mod && e.shiftKey && (e.key === '/' || e.key === '?')) {                             // 速查表
        window.dispatchEvent(new Event('wjw-term-help'));
        return false;
      }
  ...
  const exportLog = () => {
    ...
    // 弹保存对话框让用户选目录（不支持的环境回落到直接下载）
    saveBlob(blob, `terminal-${name}-${ts}.log`).then((ok) => {
      if (ok) message.success('日志已保存');
    });
  };
  ...
  return (
    <div
      ...
      onDragOver={(e) => {
        ...
        if (types.includes('application/x-wjw-asset')) {
          e.preventDefault();
